...
fun QuoteScreen(modifier: Modifier = Modifier, viewModel: QuoteViewModel = hiltViewModel()) {
    ...
    if (items is QuoteUiState.Success) {
        QuoteScreen(
            items = (items as QuoteUiState.Success).data,
            onSave = viewModel::addQuote,
            modifier = modifier
        )
    }
}
...
internal fun QuoteScreen(
    ...
    Column(modifier) {
        var nameQuote by remember { mutableStateOf("Compose") }
        Row(
            ...
            TextField(
                value = nameQuote,
                onValueChange = { nameQuote = it }
            )

            Button(modifier = Modifier.width(96.dp), onClick = { onSave(nameQuote) }) {
                Text("Save")
            }
        }
        items.forEach {
...
private fun DefaultPreview() {
    MyApplicationTheme {
        QuoteScreen(listOf("Compose", "Room", "Kotlin"), onSave = {})
    }
}
